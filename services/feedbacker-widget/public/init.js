function init (apiKey) {
  async function handleLoadWidget () {
    ...
    const WIDGET_URL = `https://andersonbosa-feedbacker-widget.netlify.app?api_key=${apiKey}&page=${page}&fingerprint=${fingerprint.visitorId}`
    const config = { method: 'HEAD' }
    const res = await fetch(`https://backend-andersonbosa.vercel.app/apikey/exists?apikey=${apiKey}`, config)
